...
//用来保存字库基本信息，地址，大小等
_font_info ftinfo; //信息头已然事先写入flash当中了
...
{
	u32 flashaddr=0;				//用于作碟，指向flash当中的相应位置，供大循环使用				    
	FIL * fftemp;
	u8 *tempbuf;
 	u8 res;	
	u16 bread;
...
{	
	...
	buf=mymalloc(SRAMIN,4096);	//申请4K字节内存  必须要事先申请内存，否则或许会成为野指针，造成不可逆的损毁
	fftemp=(FIL*)mymalloc(SRAMIN,sizeof(FIL));	//分配内存	
	...
	strcat((char*)pname,(char*)GBK24_PATH); 
 	res=f_open(fftemp,(const TCHAR*)pname,FA_READ); //res用于判断文件是否已经打开，接受文件指针
	...
	{  
		...
		for(i=0;i<FONTSECSIZE;i++)	//先擦除字库区域,提高写入速度
		{
			...
			W25QXX_Read((u8*)buf,((FONTINFOADDR/4096)+i)*4096,4096);/*读出整个扇区的内容 buf既然为4字节的
			而flash读却只能为1个字节，这种强制类型转换就是为了让读取一点一点进行下去，直到把整个4字节填满*/
			for(j=0;j<1024;j++)//校验数据
			{
				if(buf[j]!=0XFFFFFFFF)break;//需要擦除 buf是一个4字节的指针 	  
			}
			if(j!=1024)W25QXX_Erase_Sector((FONTINFOADDR/4096)+i);	//当遍历完所有的4字节后发现有非为全F的，则需要擦除该扇区
		}//这段代码可以减少无谓的擦写
		myfree(SRAMIN,buf);
...
{		
	...
	{
		t++;//↓向fontinfo中写入font info个信息
		W25QXX_Read((u8*)&ftinfo,FONTINFOADDR,sizeof(ftinfo));//读出ftinfo结构体数据，该变量在本文件的开头就已经被定义
		if(ftinfo.fontok==0XAA)break;//FONTINFOADDR为外部flash中字库所放的位置
		delay_ms(20);
	}
