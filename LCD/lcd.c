...
//LCD的画笔颜色和背景色	   
u16 POINT_COLOR=0XFE80;	//画笔颜色 出厂色为黑 0x0000，现为黄色0XFE80
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	else if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510)return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));//ILI9341/NT35310/NT35510需要公式转换一下//本实验所使用的屏幕ili9341
	else return LCD_BGR2RGB(r);						//其他IC
}			 

//针对NT35510的产品优化，删减尽可能多的冗余操作
//读取个某点的颜色值	 
//x,y:坐标
//返回值:此点的颜色
//每个颜色是16位2字节的！uint16_t系列
u16 LCD_Fast_ReadPoint(u16 x,u16 y)
{//匹配NT35510的RGB565模式
 	u16 r=0,g=0,b=0;	   
	LCD_SetCursor(x,y);	    
	LCD_WR_REG(0X2E00);	//5510 发送读GRAM指令
	opt_delay(2);	  //NT35510要分2次读出
		b=LCD_RD_DATA(); 
		g=r&0XFF;		//对于9341/5310/5510,第一次读取的是RG的值,R在前,G在后,各占8位
		g<<=8;										
return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));//ILI9341/NT35310/NT35510需要公式转换一下//本实验所使用的屏幕驱动为NT35510
}
...
//在指定区域内填充单个颜色的边框
//(sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1) s:start; e:end  
//strokeWidth：边框的宽度
//color:要填充的颜色
//画点是连续的，务必要设置清楚cursor的位置
void LCD_DrawBorder(u16 sx,u16 sy,u16 ex,u16 ey,u8 strokeWidth ,u16 color)
{
	u16 i,j;//i为行像素号，j为列像素号
	u16 downOffset=0; 
	u16 temp;
	u16 yStrictUp=sy+strokeWidth+downOffset;//事先一次性计算，精简运行时间
	u16 yStrictDown=ey-strokeWidth+downOffset;
	u16 xStrictLeft=sx+strokeWidth;
	u16 xSrtictRight=ex-strokeWidth;
	if((lcddev.id==0X6804)&&(lcddev.dir==1))	//6804横屏的时候特殊处理  
	{
		temp=sx;
		sx=sy;
	...
	{	 
		for(i=sy;i<=ey;i++){
			for(j=sx;j<ex;j++){
				if(((i>yStrictUp)&&(i<yStrictDown))&&((j>xStrictLeft)&&(j<xSrtictRight)))continue;
				LCD_Fast_DrawPoint(j,i,color);//快速画点
			}				
		}
	}
}
...
{  							  
	...
	for(t=0;t<csize;t++)//一个字符所占有的字节数 只读取足够的字节数即可，真正的索引是字节数！！可以预先设好充足的字节数，只要在主控时不读即可！！
	{   
		...
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);//当前点有值
			else if(mode==0)LCD_Fast_DrawPoint(x,y,BACK_COLOR);//着背景色，若为0，即非叠加模式，在当前点为0时着背景色
			temp<<=1;//向高位移一位
			y++;//取模方式为从上到下，自左至右
...
//size:字体大小 12/16/24
//mode:叠加方式(1)还是非叠加方式(0)
void LCD_ShowChar_Ex(u16 x,u16 y,u8 num,u8 size,u8 mode)
{  							  
  u16 temp,t1,t;//防溢出
	u16 y0=y;
	uint8_t* pChar;//承接放大后的字符的存放地址
	u16 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
 	num=num-' ';//得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库）
	pChar = zoomChar(12,24,size/2,size,(uint8_t*)&asc2_2412[num],0);
	for(t=0;t<csize;t++)//一个字符所占有的字节数 只读取足够的字节数即可，真正的索引是字节数！！可以预先设好充足的字节数，只要在主控时不读即可！！
	{   
		
		temp = pChar[t];//我TM是个天才吗，这个绝妙的简单的语句包含了传递
...
//在指定位置显示一个字符(RTC时钟显示专供)
//x,y:起始坐标
//num:要显示的字符:" "--->"~"
//size:字体大小 12/16/24
//mode:叠加方式(1)还是非叠加方式(0)
void LCD_ShowChar_RTC(u16 x,u16 y,u8 num,u8 size,u8 mode)
{  							  
  u8 temp,t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
	...
	for(t=0;t<csize;t++)//一个字符所占有的字节数 只读取足够的字节数即可，真正的索引是字节数！！可以预先设好充足的字节数，只要在主控时不读即可！！
	{   
		if(size==60&&asc2_s6030!=NULL)temp=asc2_s6030[num*csize+t];	 	//调用6030字体//非法的数组
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);//当前点有值
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum_RTC(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode)
{  
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode&0X80)LCD_ShowChar_RTC(x+(size/2)*t,y,'0',size,mode&0X01);  
				else LCD_ShowChar_RTC(x+(size/2)*t,y,' ',size,mode&0X01);  
 				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar_RTC(x+(size/2)*t,y,temp+'0',size,mode&0X01); 
	}
} 
...
//size:字体大小
//*p:字符串起始地址
void LCD_ShowStringFloat(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p)
{         
	u8 x0=x;
	width+=x;
	height+=y;
